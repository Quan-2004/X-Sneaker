import { getFirebaseDatabase, getFirebaseAuth } from '../firebase-config.js';
import { ref, onValue, set, update } from "https://www.gstatic.com/firebasejs/12.8.0/firebase-database.js";
...
async function saveGeneralSettings() {
    ...
    try {
        ...
        await update(settingsRef, generalData);
...
async function savePaymentSettings() {
    ...
    try {
        ...
        await update(settingsRef, paymentData);
...
async function saveNotificationSettings() {
    ...
    try {
        ...
        await update(settingsRef, notificationData);
...
async function exportData() {
    ...
    try {
        const snapshot = await onValue(ref(db), (snap) => {
            const data = snap.val();
...
function loadBrandSettings() {
    onValue(ref(db, 'settings/brandCustomization'), (snapshot) => {
        const data = snapshot.val() || {};
        ...
        if (banners.heroSlider) {
            renderSliderImagesList(banners.heroSlider);
        }
    });
}
