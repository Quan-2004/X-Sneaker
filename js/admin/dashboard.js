...
function loadRevenueAndOrders() {
    ...
    onValue(ordersRef, (snapshot) => {
        ...
        if (revChangeEl) {
            ...
            revChangeEl.innerHTML = `
                <span class="material-symbols-rounded text-[14px]">trending_${change >= 0 ? 'up' : 'down'}</span> ${Math.abs(change)}%
            `;
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                if (user.createdAt) {
                    ...
                    if (userMonth === currentMonth && userYear === currentYear) {
                        newThisMonth++;
                    } else if (userMonth === previousMonth && userYear === previousYear) {
                        previousMonthNew++;
                    }
                }
            });
            ...
            if (topProducts.length === 0) {
                topProductsContainer.innerHTML = `
                    <div class="text-center py-8 text-slate-400">
                        <span class="material-symbols-rounded text-4xl opacity-20">inventory_2</span>
                        <p class="text-sm mt-2">Chưa có dữ liệu bán hàng</p>
                    </div>
                `;
            } else {
    ...
    onValue(ordersRef, (snapshot) => {
        if (snapshot.exists()) {
            snapshot.forEach((childSnapshot) => {
                ...
                activities.push({
                    ...
                    icon: 'shopping_bag',
                    iconBg: 'emerald',
                    title: `New order ${order.orderId || childSnapshot.key}`,
        ...
        if (recentActivities.length === 0) {
            activitiesContainer.innerHTML = `
                <div class="text-center py-8 text-slate-400">
                    <span class="material-symbols-rounded text-4xl opacity-20">notifications</span>
                    <p class="text-sm mt-2">Chưa có hoạt động nào</p>
                </div>
            `;
        } else {
                            ...
                            <div class="flex-1 flex flex-col items-center gap-2 group cursor-pointer">
                                <div class="relative w-full">
                                    <div class="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-900 dark:bg-slate-700 text-white text-xs px-2 py-1 rounded whitespace-nowrap">
                                        ...
                                        <span class="text-slate-300">${data.orders} orders</span>
                                    </div>
                                    <div class="w-full bg-gradient-to-t from-primary to-primary/50 rounded-t-lg transition-all group-hover:from-primary-600 group-hover:to-primary/70" 
